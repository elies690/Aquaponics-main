...
class SystemController():
        ...
        def __init__(self,alarm,T_range,TDS_range,sendMessage=None,updateWebpage=None):
                ...
                self.t_high = None
                self.t_low = None
                self._timeBackUp = 'Data/Level/BackUp.csv'
                self._levelLogging = 'Data/Level/Logging/'
                self._pumpsLogging = 'Data/Pumps/Logging'
                self.sendMessage = sendMessage
                self.updateWebpage = updateWebpage
                self.nb_levels = 1
                self.levels = [self.nb_levels]
                self.time_interval_down = 30
                self.time_interval_up = 30
        ...
        def level_check(self):
                ...
                for level in self.levels:
                        if level == 'low':
                                t_down = self._get_time_backup(i,2)
                                dt = (datetime.now()-t_down).total_seconds()/60
                                if dt > self.time_interval_down:
                                        warning_msg+='level '+str(i)+ ' down too long'

                        elif level == 'up':
                                t_up = self._get_time_backup(i,0)
                                dt = (datetime.now()-t_up).total_seconds()/60
                                if dt > self.time_interval_up:
                                        warning_msg+='level '+str(i)+ ' high too long'


                if warning_msg == '':
                        asyncio.ensure_future(self.updateWebpage('warnings','level;'+'level warning'))
                else:
                        asyncio.ensure_future(self.updateWebpage('warnings','level;'+warning_msg))
