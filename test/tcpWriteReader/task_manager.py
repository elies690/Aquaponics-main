...
def schedule_task(task: Task):
    ...
    if task.DelayTimeOrder == TimeOrder.SECOND:
        schedule.every(task.Delay).seconds.do(task.Action).tag(task.Tags,'tag')
    if task.DelayTimeOrder == TimeOrder.MINUTE:
        schedule.every(task.Delay).minutes.do(task.Action).tag(task.Tags,'tag')
    if task.DelayTimeOrder == TimeOrder.HOUR:
        schedule.every(task.Delay).hours.do(task.Action).tag(task.Tags,'tag')
    if task.DelayTimeOrder == TimeOrder.DAY:
        schedule.every(task.Delay).days.do(task.Action).tag(task.Tags,'tag')
